...
{
    public class EvaluacionActualizada
    {
    ...
    public class UpdateEventosAllCommand : IRequest<ResponseBase<bool>>
    {
        public IEnumerable<EvaluacionActualizada> EvaluacionesActualizadas { get; set; }
    }

    public class UpdateEventosAllCommandHandler : IRequestHandler<UpdateEventosAllCommand, ResponseBase<bool>>
    {
        ...
        public async Task<ResponseBase<bool>> Handle(UpdateEventosAllCommand request, CancellationToken cancellationToken)
        {
            List<ParametroDTO> parametroIndicadores = GetParametroIndicadores();
            var errores = new List<string>();

            foreach (var item in request.EvaluacionesActualizadas)
            {
                if (item.Estado == 3)
                {
                    errores.Add($"La OC: {item.NumOc} con el Lote {item.NumeroLote}: Ya se encuentra en estado Enviado");
                    continue;
                }
